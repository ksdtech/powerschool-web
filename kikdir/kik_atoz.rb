...
class AToZExporter
  ...
  EXPORT_HEADER = [
    ...
    'S1 Email',
    'S1 Phone',
    'S1 Teacher Honorific',
    'S1 Teacher First Name',
    'S1 Teacher Last Name',
    'S1 Teacher Email',
    'S1 Teacher Phone',
    'S1 Teacher Room',
    'S1 Teacher Job Title',
    'H1 Address',
    'H1 City',
    'H1 State',
    'H1 ZIP',
    'H1 Home Phone',
    'H1 Parent 1 First Name',
    'H1 Parent 1 Last Name',
    'H1 Parent 1 Mobile',
    'H1 Parent 1 Work Phone',
    'H1 Parent 1 Email',
    'H1 Parent 2 First Name',
    'H1 Parent 2 Last Name',
    'H1 Parent 2 Mobile',
    'H1 Parent 2 Work Phone',
    'H1 Parent 2 Email',
    'H2 Address',
    'H2 City',
    'H2 State',
    'H2 ZIP',
    'H2 Home Phone',
    'H2 Parent 1 First Name',
    'H2 Parent 1 Last Name',
    'H2 Parent 1 Mobile',
    'H2 Parent 1 Work Phone',
    'H2 Parent 1 Email',
    'H2 Parent 2 First Name',
    'H2 Parent 2 Last Name',
    'H2 Parent 2 Mobile',
    'H2 Parent 2 Work Phone',
    'H2 Parent 2 Email'
  ]
  ...
  def parse(fname)
    CSV.foreach(fname, :col_sep => "\t", :row_sep => "\n", :headers => true,
      ...
      if !active
        ...
        # Only used in Bacich for AtoZ app
        if s[:grade_level] >= 5
          s[:home_room] = nil
          s[:homeroom_teacherfirst] = nil
          s[:homeroom_teacher] = nil
          s[:homeroom_teacheremail] = nil
        end
  ...
  def output
    ...
    puts EXPORT_HEADER.join("\t")
    @listings.each do |p|
      # puts JSON.pretty_generate(s)
      p[:siblings].each do |s|
      grade_level = s[:grade_level] == 0 ? 'K' : s[:grade_level].to_s
      student = [
          s[:first_name],  # 'S1 First Name',
          s[:last_name],   # 'S1 Last Name',
          '',              # 'S1 Nickname',
          grade_level,     # 'S1 Grade',
          s[:gender],      # 'S1 Gender',
          '',              # 'S1 Email',
          '',              # 'S1 Phone',
          '',              # 'S1 Teacher Honorific',
          s[:homeroom_teacherfirst],  # 'S1 Teacher First Name',
          s[:homeroom_teacher],      # 'S1 Teacher Last Name',
          s[:homeroom_teacheremail], # 'S1 Teacher Email',
          '',              # 'S1 Teacher Phone',
          s[:home_room],   # 'S1 Teacher Room',
          ''              # 'S1 Teacher Job Title',
      ]
      home1 =
          if p[:homes].length > 0
            [ p[:homes][0][:street],       # 'H1 Address',
              p[:homes][0][:city],         # 'H1 City',
              p[:homes][0][:state],        # 'H1 State',
              p[:homes][0][:zip],          # 'H1 ZIP',
              p[:homes][0][:home_phone],   # 'H1 Home Phone',
              p[:homes][0][:mother_first], # 'H1 Parent 1 First Name',
              p[:homes][0][:mother_last],  # 'H1 Parent 1 Last Name',
              p[:homes][0][:mother_cell],  # 'H1 Parent 1 Mobile',
              '',                          # 'H1 Parent 1 Work Phone',
              p[:homes][0][:mother_email], # 'H1 Parent 1 Email',
              p[:homes][0][:father_first], # 'H1 Parent 2 First Name',
              p[:homes][0][:father_last],  # 'H1 Parent 2 Last Name',
              p[:homes][0][:father_cell],  # 'H1 Parent 2 Mobile',
              '',                          # 'H1 Parent 2 Work Phone',
              p[:homes][0][:father_email]  # 'H1 Parent 2 Email',
            ]
          else
            [ '', # 'H1 Address',
              '', # 'H1 City',
          ...
          home2 =
              if p[:homes].length > 1
                [ p[:homes][1][:street],       # 'H2 Address',
                  p[:homes][1][:city],         # 'H2 City',
                  p[:homes][1][:state],        # 'H2 State',
                  p[:homes][1][:zip],          # 'H2 ZIP',
                  p[:homes][1][:home_phone],   # 'H2 Home Phone',
                  p[:homes][1][:mother_first], # 'H2 Parent 1 First Name',
                  p[:homes][1][:mother_last],  # 'H2 Parent 1 Last Name',
                  p[:homes][1][:mother_cell],  # 'H2 Parent 1 Mobile',
                  '',                          # 'H2 Parent 1 Work Phone',
                  p[:homes][1][:mother_email], # 'H2 Parent 1 Email',
                  p[:homes][1][:father_first], # 'H2 Parent 2 First Name',
                  p[:homes][1][:father_last],  # 'H2 Parent 2 Last Name',
                  p[:homes][1][:father_cell],  # 'H2 Parent 2 Mobile',
                  '',                          # 'H2 Parent 2 Work Phone',
                  p[:homes][1][:father_email]  # 'H2 Parent 2 Email',
                ]
              else
                [ '', # 'H2 Address',
                  '', # 'H2 City',
...
ke = AToZExporter.new(export_unapproved: true)
